...
export class InMemoryDocumentStore {
    ...
    async put(collection, key, document, options = {}) {
        const bucket = this.#getOrCreate(collection);
        if (options.conditionKey !== undefined) {
            const existing = bucket.get(key);
            if (!existing || this.#isExpired(existing)) {
                throw new AdapterError('condition-failed', `Condition failed on ${collection}/${key}: document does not exist`, { context: { collection, key, conditionKey: options.conditionKey } });
            }
            const actual = existing.doc[options.conditionKey];
            if (actual !== options.conditionValue) {
                throw new AdapterError('condition-failed', `Condition failed on ${collection}/${key}: ${options.conditionKey} mismatch`, {
                    context: {
                        collection,
                        key,
    ...
    async query(collection, params) {
        ...
        for (const [key, entry] of bucket) {
            if (this.#isExpired(entry))
                continue;
            const doc = entry.doc;
            if (doc[params.partitionKey.field] !== params.partitionKey.value)
                continue;
            if (params.sortKey) {
                const value = doc[params.sortKey.field];
                if (!sortKeyMatches(value, params.sortKey.operator, params.sortKey.value))
                    continue;
            }
        ...
        const nextCursor = start + limit < matches.length && page.length > 0
            ? encodeCursor(page[page.length - 1].key)
            : null;
        return { items: page.map((m) => m.doc), nextCursor };
    }
...
function sortKeyMatches(value, operator, target) {
    switch (operator) {
        case 'eq':
            return value === target;
        case 'gt':
