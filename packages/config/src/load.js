...
export async function loadConfig(startDir = process.cwd()) {
    ...
    if (!path) {
        throw new ConfigError(`Could not find ${CONFIG_FILE} in ${startDir} or any parent directory.`, { context: { startDir } });
    }
    ...
    try {
        ...
        throw new ConfigError(`Failed to load ${path}: ${err.message}`, {
            cause: err,
            context: { path },
        });
    }
...
function formatZodError(err, path) {
