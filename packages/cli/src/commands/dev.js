...
export const devCommand = defineCommand({
    ...
    async run({ args }) {
        ...
        await http.start();
        const templateFilter = config.template === 'astro' ? '@nymbal/template-astro' : '@nymbal/template-nextjs';
        // eslint-disable-next-line no-console
        console.log(pc.cyan(`→ Starting template: ${pc.bold(templateFilter)} (pnpm filter)`));
